...
public class FloorFinishesActivity extends AppCompatActivity implements TitleDialog.TitleDialogListener{

    private static final String TAG = FloorFinishesActivity.class.getSimpleName();
    private static final int PERMISSION_REQUEST_CODE = 100;
    TextView floorFinishTitle;
    String floorFinishesTitle;
    EditText level_floorfinArea, level_floorfinRate,skirting_floorfinArea
    ,skirting_floorfinRate;

    int level_ffArea, level_rateRate, level_fAmount,  skirting_ffinArea, skirting_ffinRate, skirting_ffinAmount;
    int totalAmount;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_floor_finishes);
        floorFinishTitle = findViewById(R.id.floorFinishTitle);
        level_floorfinArea = findViewById(R.id.level_floorfinArea);
        level_floorfinRate = findViewById(R.id.level_floorfinRate);
        skirting_floorfinArea = findViewById(R.id.skirting_floorfinArea);
        skirting_floorfinRate = findViewById(R.id.skirting_floorfinRate);
    ...
    public void createPdf() throws IOException {
        ...
        String outputPath = Environment.getExternalStorageDirectory()+"/"+ floorFinishesTitle+ ".pdf"; //Specify the path where the document will be stored
        ...
        try {
            if(Build.VERSION.SDK_INT >= 23){
                if(checkPermission()){
                    ...
                    Chunk chunk = new Chunk("Description                                    AREA              UNIT      RATE     AMOUNT ", listFont);
                    Paragraph listParagraph = new Paragraph(chunk);
                    listParagraph.setAlignment(Element.ALIGN_CENTER);

                    Chunk l1 = new Chunk("A. Level or to falls not exceeding \n   15 degrees from horizontal                 "+level_ffArea+"              " +
                            "Sq.M           "+level_rateRate+"           "+level_fAmount+"", listFont);
                    Paragraph l1Paragraph = new Paragraph(l1);
//
                    Chunk l2 = new Chunk("B. Skirting 100 high                               "+skirting_ffinArea+"             Lin.M          "+skirting_ffinRate+"           "+skirting_ffinAmount+"", listFont);
                    Paragraph l2Paragraph = new Paragraph(l2);
                    listParagraph.setAlignment(Element.ALIGN_CENTER);

                    Chunk l6 = new Chunk("FLOOR FINISHES TO SUMMARY                                                 "+totalAmount+"", listFont2);
...
//
    ...
    private boolean checkPermission() {
        int result = ContextCompat.checkSelfPermission(FloorFinishesActivity.this, android.Manifest.permission.WRITE_EXTERNAL_STORAGE);
    ...
    private void requestPermission() {
        ...
        if (ActivityCompat.shouldShowRequestPermissionRationale(FloorFinishesActivity.this, android.Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
            Toast.makeText(FloorFinishesActivity.this, "Write External Storage permission allows us to do store images. Please allow this permission in App Settings.", Toast.LENGTH_LONG).show();
        } else {
            ActivityCompat.requestPermissions(FloorFinishesActivity.this, new String[]{android.Manifest.permission.WRITE_EXTERNAL_STORAGE}, PERMISSION_REQUEST_CODE);
        }
    }
...
//
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()){
            ...
            case R.id.generate_invoice:
                floorFinishesTitle = floorFinishTitle.getText().toString().trim();
                if(floorFinishesTitle.isEmpty()){
    ...
    private void getValues() {
        try {
            level_ffArea = Integer.parseInt(level_floorfinArea.getText().toString().trim());
            level_rateRate = Integer.parseInt(level_floorfinRate.getText().toString().trim());
            level_fAmount = level_ffArea * level_rateRate;

            skirting_ffinArea = Integer.parseInt(skirting_floorfinArea.getText().toString().trim());
            skirting_ffinRate = Integer.parseInt(skirting_floorfinRate.getText().toString().trim());
            skirting_ffinAmount = skirting_ffinArea * skirting_ffinRate;

            totalAmount = level_fAmount + skirting_ffinAmount;
        }catch (Exception e){
    ...
    public void applyTitle(String title) {
        floorFinishTitle.setText(title);
    }
}
